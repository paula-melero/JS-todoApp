...
router.get('/:id', auth, async (req, res) => {
    ...
    try {
        ...
        const task = await Task.findOne({ _id: req.params.id, createdBy: req.user._id });
            if(!task) return res.status(404).json('Task with the given ID was not found');
...
router.put('/:id', auth, async (req,res) => {
    ...
    try {
        ...
        const foundId = await Task.findById(id);
        if(!foundId) 
...
router.delete('/:id', auth, async (req, res) => {
    ...
    try {
        ...
        const result = await Task.findOneAndDelete({_id: req.params.id, createdBy: req.user._id });
        
        if(!result)
            return res.status(401).json('Access denied. Cannot delete task with given ID.');
